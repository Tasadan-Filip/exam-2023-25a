...
class Section1: AppCompatActivity() {
    ...
    inner class EntitiesListAdapter internal constructor(
        ...
        override fun onBindViewHolder(holder: EntityViewHolder, position: Int) {
            ...
            val buttonView = holder.viewButton
            buttonView.setOnClickListener {
                logd("View button works")
                GlobalScope.launch(Dispatchers.Main) {
                    val intent = Intent(this@Section1, EntityAllDetailsActivity::class.java)
                    intent.putExtra(EntityAllDetailsActivity.ENTITY_ID, current.id)
...
//        }
    ...
    private fun fetchData() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            val localList = entityViewModel.getAll()
            if(localList != null && localList.size != 0) {
                toast("Using local data!")
                displayEntities(localList)
            } else {
